...
For the dependent variable we are using bilateral FDI inflows. These are from the United Nations Conference on Trade and Development (UNCTAD) and were first made available in 2014 \citep{UNCTAD:2014}. We use the entire time-period available, which years 2001 to 2012, although since we use a lagged dependent variable in out model, we only estimate years 2002 to 2012. Past work that has looked at country-year FDI relationships relied on monadic data. As stated earlier, the advantage of using dyadic data is that it not only lets us model network relationships, but the disaggregation also allows us to measure changes in FDI inflows related to covariates that are at the dyad level, such as PTAs. Because FDI flows tend to be highly dispersed we use the natural log of the values to reduce the impact of outliers and increase model fit.

\subsection{Covariates}

We control for both economic and political variables. Following the literature's standard for predicting FDI inflows we include standard gravity variables. This includes the log product of the dyad's real GDP \citep{feenstra2015next} and logged euclidean distance. Generally, larger products of GDP are associated with higher levels of FDI while longer distances are associated with less FDI \citep{mayer2011notes}. One key point here is that for the purpose of model convergence the logged product of dyadic GDP has been estimated as one variable in the model, rather than being estimated separately. 

The other key economic variable is a country's trade openness, defined as the total value of exports and imports as a percentage of GPD \citep{WB}. Past research has shown that FDI and trade are compliments \citep{aizenman2006fdi} and so we expect that higher levels of trade will be associated with higher levels of FDI. A more minor economic variable included is the GDP growth rate, which has been used in past studies to stand in for the general health of a country's economy \citep{WB}. We expect that a high GDP growth rate will be associated with more FDI, both as a sender and receiver.

We include two categories of international agreement variables. The first is four dummy variables for different types of defense agreements \citep{Gibler09}. This includes defense, entente, non-aggression, and neutrality treaties. We expect these variables to positively associated with FDI inflows, particularly defense treaties since this indicates political cooperation and a lower risk of expropriation. The second international agreement variable is preferential trade agreement (PTA) depth \citep{dur2014design}. Past work has argued that PTAs represent a commitment to liberal markets that investors would favor and therefore would be associated with increased FDI inflows \citep{buthe2014foreign}. However, past work used a count of PTAs signed at the country level. Our work takes this further by using by estimating this relationship at the dyadic level and the PTA variable we use is built using latent trait analysis with 48 different dichotomous variables regarding topics covered in PTAs. This is important since PTAs vary significantly in depth with some requiring nearly full liberalization of trade barriers while others are superficial political signals. 

There is substantial amount of work that explores the relationship between regime type and FDI inflows. The work is inconclusive \cite{johns2016under}, but we include controls for it nonetheless using the Polity IV score, which is an aggregate of institutional measures for enfranchisement and political competition.  \citep{polity2012polity}. We also include political violence to inversely proxy for state stability, which we expect to be negatively correlated with FDI inflows \citep{marshall2005major}.
