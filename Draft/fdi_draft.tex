...
For robustness checks, we re-estimate the count ERGM by pooling the data. We see in Figure \ref{fig:exog_2} that after pooling, network terms remain positive and statistically significant. In Table \ref{fig:effectPlots2}, we see that exogenous covariates are similar to yearly results and that differences when network terms are included still supports the assertion that failure to include network structure results in biased estimates. 
