...
$$Sum:\sum_{(i,j) {\in} \mathbb{Y}}\bm{y}_{i,j}$$
$$Sum, Fractional Moment:\sum_{(i,j) {\in} \mathbb{Y}}\bm{y}_{i,j}^{1/2}$$
$$Non-Zero: \bm{g}_k = \sum_{(i,j) {\in} \mathbb{Y}} \mathbb{I}(\bm{y}_{i,j} \neq 0)$$
$$ Reciprocity: \sum_{(i,j) {\in} \mathbb{Y}}min(\bm{y}_{i,j},\bm{y}_{j,i})$$
$$Transitive Ties: \bm{g_v(y)} = \sum_{(i,j) {\in} \mathbb{Y}} v_{\text{affect}} (\bm{y}_{i,j}, v_{\text{combine}}(v_{\text{2-path}}(\bm{y}_{i,k},\bm{y}_{k,j})_{k{\in}N {\backslash} \{i,j\}}))$$
