...
save.image("main_network_results_RR.RData")

#independent models
ergm_independent <- expression(ergm(net ~ sum+ sum(1/2) +nonzero+#mutual+
                                  #edgecovmutual(oecd_both)+ edgecovmutual(not_oecd_both)+
                                  #transitiveweights("min", "max", "min")+
                                  nodeicov("polity")+nodeocov("polity")+
                                  nodeicov("trade_opennes")+nodeocov("trade_opennes")+
...
ergms <- foreach(i=1:length(list_of_networks)) %dopar% {
  ...
  net %e% "tValue" <- ceiling(log(edge_vals)*2)
  edge_vals2 <- net %e% "tValue"
  eval(ergm_independent)
}
stopImplicitCluster()

print("finished independent models")

save.image("main_independent_results_RR.RData")
