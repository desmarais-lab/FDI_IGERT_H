...
The second structural dependency we model is transitivity, or the likelihood that country A, will send FDI to country C, given that country A sends FDI to country B and country B send FDI to country C. We expect this clustering given the fragmented global supply chains for production and the vertical FDI that follows. For the model transitivity is defined as: 
...
\headerbox{Network Plots}{name=network,column=1,span=2,row=0,below=results,above=references,bottomaligned=theory}{
...
\headerbox{Interpretation}{name=results2,column=3,below=data,bottomaligned=theory}{ 
